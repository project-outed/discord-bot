...
class MemberMonitorView(discord.ui.LayoutView):
    def __init__(self, bot: discord.Client = None, data=None):
        super().__init__()
        self.bot = bot
        self.data = data
        ...
        if reports:
            ...
            for report in reports:
                ...
                if isinstance(created_at, str):
                    timestamp_obj = datetime.fromisoformat(created_at.replace("Z", "+00:00"))
                else:
                    timestamp_obj = created_at

                container.add_item(
                    ui.TextDisplay(
                        "**%s**\n-# %s" % (
                            report['reason'],
                            f"Game: {report['game']} - Added on: {timestamp_obj.strftime('%Y-%m-%d %H:%M:%S')}"
                        )
                    )
                )
